...
export function Programacao() {
  ...
  const fetchData = useCallback(() => {
    client.query({
      query: gql`
          {
            allProgramacaoguaianases (first:100) {
              evento
              horario
              destaque
              mes
      ...
      .then((res) => {
        const eventsComKeys = res.data.allProgramacaoguaianases.map((event: Event) => ({
          ...event,
          key: uuidv4(),
        }));
  ...
  return (
    <AgendaContainer>
      ...
      <EventsGrid>
        ...
        console.log(event);
          return <EventCard key={event.key} event={event} onMoreInfoClick={handleOpenModal} />;
        })}
      </EventsGrid>
